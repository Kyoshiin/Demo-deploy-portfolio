...
const FeaturedProject = ({ type, title, summary, img, link, github }) => {
  return (
    <motion.div className={"relative"} whileHover={{ scale: 1.1 }}>
      <div
        className="absolute top-0 -right-3 w-[101%] h-[103%] rounded-[2.5rem] block bg-dark rounded-br-3xl dark:bg-light
        xs:-right-2 sm:h-[102%] xs:w-full xs:rounded-[1.5em]"
      />
    ...
    border-dark bg-light shadow-2xl p-12 relative dark:bg-dark dark:border-light lg:flex-col lg:p-8 xs:rounded-2xl xs:rounded-br-3xl xs:p-4"
      >
        <Link
          href={link}
          target="_blank"
          className="w-1/2 curser-pointer overflow-hidden rounded-lg lg:w-full"
        >
          <Image
            src={img}
            alt={title}
        ...
        <Layout className="pt-16">
          ...
          <div className="grid grid-cols-12 gap-24 gap-y-32 xl:gap-x-16 lg:gap-x-8 md:gap-y-24 sm:gap-x-0">
            <div className="col-span-12">
              <FeaturedProject
                title="Book Reader"
                summary="Native Android application for searching books using googlebooks api."
                link="/"
